...
// Patched by Harrie Martens and Hubert Vogten to  allow a name to be past
// for a link. This name will be used when no text selected.
//
// Adds a link.
	...
	if (!readOnly && url != null && url != '' && internalUpdateSelection()) {
		...
		if (document.all) {
			if (selectionRange.text == null || selectionRange.text == '') {
				selectionRange.text = name;
				nameLength = name.length;
				selectionRange.moveStart("character", -1*nameLength);
				selectionRange.select();
				setStatus(STATUS_EXEC_CMD, 1);
				setStatus(STATUS_MODIFIED, null);
			}
			if (selectionRange.execCommand('CreateLink', false,url)) {
				if (nameLength != -1) {
					selectionRange.moveStart("character", nameLength);
					selectionRange.select();
				}
				setStatus(STATUS_EXEC_CMD, 1);
				setStatus(STATUS_MODIFIED, null);
			...
			if (selection == null || selection == "") {		
				nameLength = name.length;
				var urlTextNode = editorDoc.createTextNode(name);
				selectionRange.insertNode(urlTextNode);
			
				if (selection.rangeCount > 0) {
					rOrg = selection.getRangeAt(selection.rangeCount - 1);
				}	
				selection.removeAllRanges();
				
				rTemp = editorDoc.createRange();
				rTemp.setStart(urlTextNode,0);
				rTemp.setEnd(urlTextNode,nameLength);
				selection.addRange(rTemp);

			}			
			if (editorDoc.execCommand('createlink', false, url)) {
				if (rOrg != null) {
					selection.removeAllRanges();
					selection.addRange(rOrg);
				}	
				setStatus(STATUS_EXEC_CMD, 1);
				setStatus(STATUS_MODIFIED, null);
			}
